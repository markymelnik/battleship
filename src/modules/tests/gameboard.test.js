...
test('createGameBoard creates a 10x10 array filled with null', () => {

  expect((gameboard.createGameBoard()).length).toBe(10);

  (gameboard.createGameBoard()).forEach((row) => {
...
test('createFleet returns an empty array', () => {
  expect(gameboard.createFleet()).toEqual([]);
});
...
test('check for valid vertical and horizontal placement', () => {
  expect(gameboard.validPlacement([5, 5], Ship('carrier'), 'vertical')).toBe(
    true
  );
  expect(gameboard.validPlacement([6, 5], Ship('carrier'), 'vertical')).toBe(
    false
  );
  expect(gameboard.validPlacement([5, 5], Ship('carrier'), 'horizontal')).toBe(
    true
  );
  expect(gameboard.validPlacement([5, 6], Ship('carrier'), 'horizontal')).toBe(
    false
  );
  expect(gameboard.validPlacement([9, 3], Ship('destroyer'), 'vertical')).toBe(
    false
  );
  expect(
    gameboard.validPlacement([2, 7], Ship('battleship'), 'horizontal')
  ).toBe(false);
  expect(gameboard.validPlacement([7, 2], Ship('submarine'), 'vertical')).toBe(
    true
  );
  expect(
    gameboard.validPlacement([2, 7], Ship('submarine'), 'horizontal')
  ).toBe(true);
});
...
test('check that ships do not cross over each other', () => {
  ...
  expect(
    gameboard.isPathClearOfShips([3, 2], Ship('battleship'), 'vertical')
  ).toBe(false);
  expect(
    gameboard.isPathClearOfShips([3, 4], Ship('submarine'), 'vertical')
  ).toBe(false);
  expect(
    gameboard.isPathClearOfShips([3, 6], Ship('cruiser'), 'vertical')
  ).toBe(true);
});

test('check that all valid adjacent tiles are returned', () => {
  expect(gameboard.getAdjacentTiles([5, 5])).toStrictEqual([
    [4, 4],
    [4, 5],
    [4, 6],
    [5, 6],
  ...
  expect(gameboard.getAdjacentTiles([1, 0])).toStrictEqual([
    [0, 0],
    [0, 1],
    [1, 1],
    [2, 1],
  ...
  expect(gameboard.getAdjacentTiles([0, 0])).toStrictEqual([
    [0, 1],
    [1, 1],
    [1, 0],
  ]);
...
test('check that all adjacent tiles of a ship to be placed are empty', () => {
  expect(
    gameboard.areAdjacentTilesEmpty([4, 2], Ship('battleship'), 'horizontal')
  ).toBe(true);
  gameboard.placeShip([3, 3], Ship('carrier'), 'horizontal');
  expect(
    gameboard.areAdjacentTilesEmpty([2, 2], Ship('battleship'), 'horizontal')
  ).toBe(false);
  expect(
    gameboard.areAdjacentTilesEmpty([2, 8], Ship('battleship'), 'vertical')
  ).toBe(false);
  expect(
    gameboard.areAdjacentTilesEmpty([2, 9], Ship('battleship'), 'vertical')
  ).toBe(true);
});
...
test('gameboard position contains the ship object when ship is placed', () => {
  ...
  expect(JSON.stringify(gameboard.board[4][2])).toEqual(
    JSON.stringify(Ship('destroyer'))
  );
  expect(JSON.stringify(gameboard.board[4][3])).toEqual(
    JSON.stringify(Ship('destroyer'))
  );
  ...
  expect(JSON.stringify(gameboard.board[3][8])).toEqual(
    JSON.stringify(Ship('submarine'))
  );
  expect(JSON.stringify(gameboard.board[4][8])).toEqual(
    JSON.stringify(Ship('submarine'))
  );
  expect(JSON.stringify(gameboard.board[5][8])).toEqual(
    JSON.stringify(Ship('submarine'))
  );
...
test('the same type of ship cannot be placed twice', () => {
  ...
  expect(() => {
    gameboard.placeShip([6, 4], Ship('destroyer'), 'horizontal');
  }).toThrow(Error);
});
...
test('check to see if whole fleet is sunk, signaling the game ending', () => {
  ...
  expect(JSON.stringify(gameboard.fleet)).toBe(
    JSON.stringify([Ship('battleship')])
  );
  gameboard.receiveAttack([1, 8]);
  gameboard.receiveAttack([2, 8]);
  gameboard.receiveAttack([3, 8]);
